...
bool ParticleShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"./particle.vs");
	if(error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"./particle.ps");
	if(error != 0)
	{
		return false;
	}
...
bool ParticleShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, ID3D11ShaderResourceView* texture)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, texture);
	if(!result)
	{
		return false;
	}
...
bool ParticleShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
