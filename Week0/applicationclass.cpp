...
bool ApplicationClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	ID3D11Device* a = m_Direct3D->GetDevice();
	result = m_ParticleSystem->Initialize(a, m_Direct3D->GetDeviceContext(), textureFilename);
	if(!result)
	{
		MessageBoxW(hwnd, L"Could not initialize the particle system object.", L"Error", MB_OK);
		return false;
	}
	...
	if(!result)
	{
		MessageBoxW(hwnd, L"Could not initialize the particle shader object.", L"Error", MB_OK);
		return false;
	}
...
bool ApplicationClass::Render()
{
    ...
    result = m_ParticleShader->Render(m_Direct3D->GetDeviceContext(), m_ParticleSystem->GetIndexCount(), worldMatrix, m_ParticleSystem->GetTexture());
	if(!result)
	{
		return false;
	}
